...
class _netG(nn.Module):
    def __init__(self, opt, nclasses):
        ...
        self.main = nn.Sequential(
            nn.ConvTranspose2d(self.nz+self.ndim+self.nclasses+1,
                               self.ngf, 2, 1, 0, bias=True),  # (2, 2)
            nn.BatchNorm2d(self.ngf, affine=True),
            # nn.LeakyReLU(0.2, inplace=True),

            nn.ConvTranspose2d(self.ngf, (self.ngf // 2), 4,
                               2, 1, bias=True),    # (4, 4)
            nn.BatchNorm2d((self.ngf // 2), affine=True),
            # nn.LeakyReLU(0.2, inplace=True),

            nn.ConvTranspose2d((self.ngf // 2), (self.ngf // 4),
                               (4, 3), 2, 1, bias=True),    # （8，7）
            nn.BatchNorm2d((self.ngf // 4), affine=True),
            # nn.LeakyReLU(0.2, inplace=True),

            nn.ConvTranspose2d((self.ngf // 4), 1, 4, 3, 1,
